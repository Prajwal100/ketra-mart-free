...
@section('content')
    ...
    @endif
    @if (session()->has('recently_view') && count(session()->get('recently_view')) > 0)
        <!-- Recently Viewed -->
            <section class="products-area recently-viewed pb-5">
                <div class="container">
                    <div class="section-title">
                        <!-- <span class="sub-title">Shop Now</span> -->
                        <h2>Recently Viewed
                    ...
                    <div class="owl-carousel owl-theme products-slider">
                        @foreach (session()->get('recently_view') as $item)
                            <div class="item">
                                    ...
                                    <div class="products-image">
                                        ...
                                        <a href="{{ route('product.detail', $item->slug) }}">
                                            <img style="border:1px solid #ddd"
                                                 src="{{ $item->featured_image != null ? asset($item->featured_image) : Helper::DefaultImage() }}"
                                                 class="main-image" alt="image">
                                            <img style="border:1px solid #ddd"
                                                 src="{{ $image[0] != null ? asset($image[0]) : Helper::DefaultImage() }}"
                                    ...
                                    <div class="products-content">
                                        <div class="wishlist-btn">
                                            <a href="javascript:;" class="add_to_wishlist"
                                               data-product-id="{{ $item->id }}" id="add_to_wishlist_{{ $item->id }}">
                                                @php
                                                    if(\Illuminate\Support\Facades\Auth::check()){
                                                        $already_wishlist=\App\Models\Wishlist::where('user_id',auth()->user()->id)->where('product_id',$item->id)->first();
                                                    }
                                                     else{
                                                                 $already_wishlist=null;
                                                             }
                                        ...
                                        <div class="price">
                                            <span
                                                class="new-price">{{ Helper::currency_converter($item->purchase_price) }}</span>
                                        </div>
                                        ...
                                        <a href="javascript:" class="add-to-cart" data-quantity="1"
                                           data-price="{{ $item->purchase_price }}" data-id="{{ $item->id }}"
                                           id="add_to_cart{{ $item->id }}">Add to Cart</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="container">
                <div class="products-review-form">
                    <div class="review-title">
                        <h3 class="d-flex"><i class='bx bx-message-detail bx-tada d-inline-block d-sm-none mr-1'
                                              style='color:#474747'></i> Reviews</h3>
                        <a href="#" class="default-btn primary-btn text-capitalize" data-toggle="modal"
                           data-target="#reviewModal">
                            <span> Write Review </span>
                            <i class="bx bx-right-arrow-alt ml-3"></i>
                        </a>
                    </div>
                    <div class="review-comments" id="reviews">
                        @if (count($display_reviews) > 0)
                            @include('frontend.partials._review_list',['reviews'=>$display_reviews])
                        @else
                            <p class="text-secondary pb-4 border mt-2 p-2 text-center pt-4"><i
                                    class="fas fa-comments"></i> Be the first to write a review of this product.</p>
                        @endif
                    </div>
                    @if (count($reviews) > 5)
                        <div class="view-more-review_item mb-4" id="ShowReviewItem">
                            <a href="javascript:" class="load-more"
                               data-totalResult="{{ \App\Models\ProductReview::where('status', 'active')->where('product_id', $product->id)->count() }}"><span
                                    class="bx bx-chevron-down"></span> View More </a>
                        </div>
                    @endif
